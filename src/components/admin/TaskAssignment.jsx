...
export default function TaskAssignmentManagement() {
  ...
  const apiService = {
    ...
    rotations: {
      getRotations: async () => {
        try {
          console.log('[GET ROTATIONS] Fetching rotations...');
          console.log('[GET ROTATIONS] URL:', `${TASK_ASSIGNMENT_URL}/rotations/`);
          console.log('[GET ROTATIONS] Auth Token:', authToken ? 'Present' : 'Missing');
          ...
          console.log('[GET ROTATIONS] Response Status:', response.status);
          console.log('[GET ROTATIONS] Response OK:', response.ok);

          if (!response.ok) {
            const errorData = await response.json().catch(() => ({}));
            console.error('[GET ROTATIONS] Error Response:', errorData);
            throw new Error(errorData.message || 'Failed to fetch rotations');
          }

          const data = await response.json();
          console.log('[GET ROTATIONS] Success! Retrieved data:', data);
          console.log('[GET ROTATIONS] Number of rotations:', data.rotations?.length || 0);

          return data;
        } catch (error) {
          console.error('[GET ROTATIONS] Exception:', error);
          console.error('[GET ROTATIONS] Error message:', error.message);
          throw error;
        }
      },
      ...
      createRotation: async (rotationData) => {
        try {
          console.log('[CREATE ROTATION] Creating new rotation...');
          console.log('[CREATE ROTATION] URL:', `${TASK_ASSIGNMENT_URL}/rotations/`);
          console.log('[CREATE ROTATION] Request Data:', rotationData);
          console.log('[CREATE ROTATION] Request Data (stringified):', JSON.stringify(rotationData, null, 2));
          console.log('[CREATE ROTATION] Auth Token:', authToken ? 'Present' : 'Missing');
          ...
          console.log('[CREATE ROTATION] Response Status:', response.status);
          console.log('[CREATE ROTATION] Response OK:', response.ok);

          if (!response.ok) {
            const errorData = await response.json().catch(() => ({}));
            console.error('[CREATE ROTATION] Error Response:', errorData);
            console.error('[CREATE ROTATION] Validation Errors:', errorData.errors);
            throw new Error(errorData.message || 'Failed to create rotation');
          }

          const data = await response.json();
          console.log('[CREATE ROTATION] Success! Created rotation:', data);
          console.log('[CREATE ROTATION] New Rotation ID:', data.rotation?.id);

          return data;
        } catch (error) {
          console.error('[CREATE ROTATION] Exception:', error);
          console.error('[CREATE ROTATION] Error message:', error.message);
          throw error;
        }
      },
      ...
      updateRotation: async (rotationId, rotationData) => {
        try {
          console.log('[UPDATE ROTATION] Updating rotation...');
          console.log('[UPDATE ROTATION] Rotation ID:', rotationId);
          console.log('[UPDATE ROTATION] URL:', `${TASK_ASSIGNMENT_URL}/rotations/${rotationId}/`);
          console.log('[UPDATE ROTATION] Request Data:', rotationData);
          console.log('[UPDATE ROTATION] Request Data (stringified):', JSON.stringify(rotationData, null, 2));
          console.log('[UPDATE ROTATION] Auth Token:', authToken ? 'Present' : 'Missing');
          ...
          console.log('[UPDATE ROTATION] Response Status:', response.status);
          console.log('[UPDATE ROTATION] Response OK:', response.ok);

          if (!response.ok) {
            const errorData = await response.json().catch(() => ({}));
            console.error('[UPDATE ROTATION] Error Response:', errorData);
            console.error('[UPDATE ROTATION] Validation Errors:', errorData.errors);
            throw new Error(errorData.message || 'Failed to update rotation');
          }

          const data = await response.json();
          console.log('[UPDATE ROTATION] Success! Updated rotation:', data);
          console.log('[UPDATE ROTATION] Updated Rotation ID:', data.rotation?.id);

          return data;
        } catch (error) {
          console.error('[UPDATE ROTATION] Exception:', error);
          console.error('[UPDATE ROTATION] Error message:', error.message);
          throw error;
        }
      },
      ...
      deleteRotation: async (rotationId) => {
        try {
          console.log('[DELETE ROTATION] Deleting rotation...');
          console.log('[DELETE ROTATION] Rotation ID:', rotationId);
          console.log('[DELETE ROTATION] URL:', `${TASK_ASSIGNMENT_URL}/rotations/${rotationId}/`);
          console.log('[DELETE ROTATION] Auth Token:', authToken ? 'Present' : 'Missing');
          ...
          console.log('[DELETE ROTATION] Response Status:', response.status);
          console.log('[DELETE ROTATION] Response OK:', response.ok);

          if (!response.ok) {
            const errorData = await response.json().catch(() => ({}));
            console.error('[DELETE ROTATION] Error Response:', errorData);
            throw new Error(errorData.message || 'Failed to delete rotation');
          }

          const data = await response.json();
          console.log('[DELETE ROTATION] Success! Deleted rotation:', data);
          console.log('[DELETE ROTATION] Deleted Rotation Info:', data.deleted_rotation);

          return data;
        } catch (error) {
          console.error('[DELETE ROTATION] Exception:', error);
          console.error('[DELETE ROTATION] Error message:', error.message);
          throw error;
        }
      }
    },
  ...
  const handleEditRotation = (rotation) => {
    console.log('[EDIT ROTATION] Rotation data:', rotation);
    console.log('[EDIT ROTATION] Tasks detail:', rotation.tasks_detail);
    console.log('[EDIT ROTATION] Tasks:', rotation.tasks);

    setSelectedRotation(rotation);
    const taskIds = (rotation.tasks_detail || rotation.tasks || [])
      .map(t => t.id)
      .filter(id => id !== undefined && id !== null);

    console.log('[EDIT ROTATION] Extracted task IDs:', taskIds);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <div className="flex items-center gap-2 mt-2">
              <span className={`px-2 py-1 rounded text-xs font-medium ${currentUser.role === 'admin' ? 'bg-purple-100 text-purple-800' :
      ...
      <div className="mb-6">
        <div className="flex flex-wrap gap-2 border-b border-gray-200">
          <button
            ...
            className={`px-4 py-2 font-medium text-sm rounded-t-lg transition-colors ${activeView === 'my-assignments'
          ...
          {(currentUser.role === 'admin' || currentUser.role === 'supervisor') && (
            <>
              <button
                ...
                className={`px-4 py-2 font-medium text-sm rounded-t-lg transition-colors ${activeView === 'all-assignments'
                  ? 'bg-blue-600 text-white'
                  : 'bg-white text-gray-600 hover:text-gray-900 hover:bg-gray-50'
                  }`}
              >
              ...
              <button
                ...
                className={`px-4 py-2 font-medium text-sm rounded-t-lg transition-colors ${activeView === 'rotations'
                  ? 'bg-blue-600 text-white'
                  : 'bg-white text-gray-600 hover:text-gray-900 hover:bg-gray-50'
                  }`}
              >
                    ...
                    {rotations.length === 0 ? (
                      ...
                      rotations.map((rotation) => (
                        <tr key={rotation.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 whitespace-nowrap">
                            <span className={`px-3 py-1 rounded-full text-xs font-medium ${rotation.is_active
